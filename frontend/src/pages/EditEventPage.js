...
function EditEventPage() {
  ...
  const loadEventData = async () => {
    try {
      ...
      const result = await api.getHikeById(id);
      ...
      if (result) {
        ...
        setEventData({
          ...
          status: result.status || 'gathering_interest',
          cost: result.cost || 0,
          image_url: result.image_url || '',
          price_is_estimate: result.price_is_estimate || false,
          date_is_estimate: result.date_is_estimate || false,
          location_link: result.location_link || '',
          destination_website: result.destination_website || '',
          gps_coordinates: result.gps_coordinates || '',
          registration_deadline: formatDateTimeLocal(result.registration_deadline),
          payment_deadline: formatDateTimeLocal(result.payment_deadline),
          registration_closed: result.registration_closed || false,
          pay_at_venue: result.pay_at_venue || false
        });

        // Load event type and event type data
        if (result.event_type) {
          setEventType(result.event_type);
        }
        if (result.event_type_data) {
          // Ensure default values for required fields if they're missing
          const typeData = result.event_type_data;
          if (result.event_type === 'hiking' && !typeData.hike_type) {
            typeData.hike_type = 'Day Hike';
          }
          if (result.event_type === 'hiking' && !typeData.difficulty) {
            typeData.difficulty = 'Moderate';
          }
        ...
        // Load tags
        try {
          const tagsResponse = await api.getEventTags(id);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const result = await api.updateHike(id, submissionData, token);
      ...
      if (result.success) {
        ...
        try {
          ...
          if (!tagResult.success) {
            console.error('❌ Tag update failed:', tagResult);
            setError('Event updated but failed to save tags: ' + (tagResult.error || 'Unknown error'));
            return;
          }
        } catch (tagError) {
          console.error('❌ Failed to update tags:', tagError);
          setError('Event updated but failed to save tags: ' + (tagError.message || 'Unknown error'));
          return;
        }

        // Navigate back to where user came from (event details or manage events)
        const referrer = location.state?.from;
        if (referrer === 'event-details') {
          navigate(`/hike/${id}`);
        } else {
                    ...
                    <div className="col-md-6">
                      <div className="form-check">
                        <input
                          ...
                          disabled={loading}
                        />
                        <label className="form-check-label" htmlFor="payAtVenueEdit">
